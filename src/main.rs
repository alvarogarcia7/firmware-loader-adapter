...
use config::Config;
use session::{get_default_session_path, Session, SessionManager};
...
enum Commands {
    ...
    Login {
        ...
        #[arg(
            short,
            long,
            help = "Serial port (e.g., /dev/ttyUSB0 or COM3) - overrides config"
        )]
        port: Option<String>,
        ...
        #[arg(
            short,
            long,
            help = "Baud rate for serial connection - overrides config"
        )]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Login {
            ...
            handle_login(username, port, baud_rate, register, &config).await?;
        }
        Commands::ListFiles { directory } => {
            handle_list_files(directory, &config).await?;
        }
...
async fn handle_login(
    ...
    let baud_rate = baud_rate.unwrap_or(config.baud_rate);
    ...
    if register {
        ...
        println!(
            "Session created for user '{}' on port {} at {} baud",
            username, port, baud_rate
        );
    } else {
        if !credentials_path.exists() {
...
async fn handle_list_files(directory: Option<PathBuf>, config: &Config) -> Result<()> {
    let session_manager = SessionManager::new(get_default_session_path());
    ...
    println!(
        "Active session: user '{}' on port {} at {} baud",
        session.username, session.port, session.baud_rate
    );

    let dir = directory.unwrap_or_else(|| config.origin_folder.clone());
...
async fn handle_send_file(session: &Session, file_path: PathBuf) -> Result<()> {
    ...
    let checksum = file_ops::FileOperations::compute_sha256(&file_path)
        .await
    ...
    let file_start_frame = protocol_handler
        .create_file_start(filename.clone(), file_size, Some(checksum.clone()))
        .context("Failed to create FileStart frame")?;

    let file_start_data = file_start_frame
        .serialize()
        .context("Failed to serialize FileStart frame")?;

    serial_transfer
        .send_message_with_retry(&file_start_data)
        .await
        .context("Failed to send FileStart message")?;

    println!("Waiting for acknowledgment...");
    let ack_data = serial_transfer
        .receive_message_with_retry()
        .await
        .context("Failed to receive FileStart acknowledgment")?;

    let ack_frame =
        protocol::Frame::deserialize(&ack_data).context("Failed to deserialize acknowledgment")?;

    if ack_frame.message_type != protocol::MessageType::Ack {
        if ack_frame.message_type == protocol::MessageType::Error {
            if let Ok(protocol::MessagePayload::Error { code, message }) =
                ack_frame.extract_payload()
            {
    ...
    loop {
        ...
        let chunk_frame = protocol_handler
            .create_file_chunk(sequence, chunk_data)
            .context("Failed to create FileChunk frame")?;

        let chunk_frame_data = chunk_frame
            .serialize()
            .context("Failed to serialize FileChunk frame")?;

        serial_transfer
            .send_message_with_retry(&chunk_frame_data)
            .await
        ...
        let ack_data = serial_transfer
            ...
            .await
            .context(format!(
                "Failed to receive acknowledgment for chunk {}",
                sequence
            ))?;
        ...
        let speed = if elapsed > 0.0 {
            bytes_sent as f64 / elapsed / 1024.0
        } else {
            0.0
        };

        print!(
            "\rProgress: {}/{} bytes ({:.1}%) - Chunk {}/{} - Speed: {:.2} KB/s",
    ...
    let file_end_frame = protocol_handler
        .create_file_end(sequence, Some(checksum.clone()))
        .context("Failed to create FileEnd frame")?;

    let file_end_data = file_end_frame
        .serialize()
        .context("Failed to serialize FileEnd frame")?;

    serial_transfer
        .send_message_with_retry(&file_end_data)
        .await
        .context("Failed to send FileEnd message")?;

    println!("Waiting for final acknowledgment...");
    let final_ack_data = serial_transfer
        .receive_message_with_retry()
        .await
    ...
    if final_ack_frame.message_type == protocol::MessageType::Error {
        if let Ok(protocol::MessagePayload::Error { code, message }) =
            final_ack_frame.extract_payload()
        {
    ...
    if final_ack_frame.message_type != protocol::MessageType::Ack {
        anyhow::bail!(
            "Expected Ack for FileEnd, got {:?}",
            final_ack_frame.message_type
        );
    }
...
async fn handle_receive_file(session: &Session, file_path: PathBuf) -> Result<()> {
    ...
    let file_start_data = serial_transfer
        .receive_message_with_retry()
        .await
    ...
    if file_start_frame.message_type != protocol::MessageType::FileStart {
        let error_frame =
            protocol_handler.create_error(1, "Expected FileStart message".to_string())?;
        let _ = serial_transfer
            .send_message(&error_frame.serialize()?)
            .await;
        anyhow::bail!(
            "Expected FileStart, got {:?}",
            file_start_frame.message_type
        );
    }

    let (filename, file_size, expected_checksum) = match file_start_frame.extract_payload()? {
        protocol::MessagePayload::FileStart {
            filename,
            file_size,
            checksum,
        ...
        _ => {
            let error_frame =
                protocol_handler.create_error(2, "Invalid FileStart payload".to_string())?;
            let _ = serial_transfer
                .send_message(&error_frame.serialize()?)
                .await;
    ...
    let ack_frame = protocol_handler
        .create_ack(0, protocol::MessageType::FileStart)
        .context("Failed to create acknowledgment")?;
    let ack_data = ack_frame
        .serialize()
        .context("Failed to serialize acknowledgment")?;

    serial_transfer
        .send_message_with_retry(&ack_data)
        .await
    ...
    loop {
        let chunk_data = serial_transfer
            .receive_message_with_retry()
            .await
            .context("Failed to receive message")?;

        let chunk_frame =
            protocol::Frame::deserialize(&chunk_data).context("Failed to deserialize frame")?;
        ...
        match chunk_frame.message_type {
            protocol::MessageType::FileChunk => {
                let (sequence, data) = match chunk_frame.extract_payload()? {
                    ...
                    _ => {
                        let error_frame = protocol_handler
                            .create_error(3, "Invalid FileChunk payload".to_string())?;
                        let _ = serial_transfer
                            .send_message(&error_frame.serialize()?)
                            .await;
                ...
                let ack_frame =
                    protocol_handler.create_ack(sequence, protocol::MessageType::FileChunk)?;
                ...
                let elapsed = start_time.elapsed().as_secs_f64();
                let speed = if elapsed > 0.0 {
                    bytes_received as f64 / elapsed / 1024.0
                } else {
                    0.0
                };
            ...
            protocol::MessageType::FileEnd => {
                let (total_chunks, checksum) = match chunk_frame.extract_payload()? {
                    protocol::MessagePayload::FileEnd {
                        total_chunks,
                        checksum,
                    } => (total_chunks, checksum),
                    _ => {
                        let error_frame = protocol_handler
                            .create_error(5, "Invalid FileEnd payload".to_string())?;
                        let _ = serial_transfer
                            .send_message(&error_frame.serialize()?)
                            .await;
                ...
                if let Some(ref expected_checksum) = expected_checksum {
                    ...
                    let computed_checksum = file_ops::FileOperations::compute_sha256(&file_path)
                        .await
                ...
                let ack_frame =
                    protocol_handler.create_ack(total_chunks, protocol::MessageType::FileEnd)?;
                let ack_data = ack_frame.serialize()?;
                serial_transfer
                    .send_message_with_retry(&ack_data)
                    .await
